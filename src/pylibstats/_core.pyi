"""Type stubs for the pylibstats._core native extension module.

Internal: users should import from ``pylibstats`` (not ``pylibstats._core``)
to get validated wrapper classes.  The public API stubs are in __init__.pyi.

Mutable distribution parameters (mu, sigma, lam, etc.) are declared as
read-write properties. The wrapper classes in __init__.py override them with
validated Python properties, but they build those by reading the nanobind
descriptors off these classes (``_core.Gaussian.mu``), so type checkers need
the declarations here.
"""

from typing import overload

import numpy as np
from numpy.typing import NDArray

# ---------------------------------------------------------------------------
# Every distribution class exposes these methods and read-only properties
# (bound by bind_common_methods<> in _core.cpp).  Per-distribution
# constructors and mutable parameters are listed on each class.
# ---------------------------------------------------------------------------
#   pdf(x), log_pdf(x), cdf(x)  — scalar and NDArray overloads
#   ppf(p)             — scalar and NDArray overloads
#   fit(data: NDArray[np.float64]) -> None
#   sample(n=1, seed=None) -> NDArray[np.float64]
#   __repr__() -> str
#
#   Properties (read-only):
#     mean, variance, std, skewness, kurtosis  : float
#     name : str
#     support : tuple[float, float]
#     is_discrete : bool
#     num_parameters : int
# ---------------------------------------------------------------------------

class Gaussian:
    def __init__(self, mu: float = 0.0, sigma: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def sigma(self) -> float: ...
    @sigma.setter
    def sigma(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Exponential:
    def __init__(self, lam: float = 1.0) -> None: ...
    @property
    def lam(self) -> float: ...
    @lam.setter
    def lam(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Uniform:
    def __init__(self, a: float = 0.0, b: float = 1.0) -> None: ...
    @property
    def a(self) -> float: ...
    @a.setter
    def a(self, value: float) -> None: ...
    @property
    def b(self) -> float: ...
    @b.setter
    def b(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Poisson:
    def __init__(self, lam: float = 1.0) -> None: ...
    @property
    def lam(self) -> float: ...
    @lam.setter
    def lam(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class DiscreteUniform:
    def __init__(self, a: int = 0, b: int = 1) -> None: ...
    @property
    def a(self) -> int: ...
    @a.setter
    def a(self, value: int) -> None: ...
    @property
    def b(self) -> int: ...
    @b.setter
    def b(self, value: int) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Gamma:
    def __init__(self, alpha: float = 1.0, beta: float = 1.0) -> None: ...
    @property
    def alpha(self) -> float: ...
    @alpha.setter
    def alpha(self, value: float) -> None: ...
    @property
    def beta(self) -> float: ...
    @beta.setter
    def beta(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Beta:
    def __init__(self, alpha: float = 1.0, beta: float = 1.0) -> None: ...
    @property
    def alpha(self) -> float: ...
    @alpha.setter
    def alpha(self, value: float) -> None: ...
    @property
    def beta(self) -> float: ...
    @beta.setter
    def beta(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class ChiSquared:
    def __init__(self, k: float = 1.0) -> None: ...
    @property
    def k(self) -> float: ...
    @k.setter
    def k(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class StudentT:
    def __init__(self, nu: float = 1.0) -> None: ...
    @property
    def nu(self) -> float: ...
    @nu.setter
    def nu(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class LogNormal:
    def __init__(self, mu: float = 0.0, sigma: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def sigma(self) -> float: ...
    @sigma.setter
    def sigma(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Pareto:
    def __init__(self, scale: float = 1.0, alpha: float = 1.0) -> None: ...
    @property
    def scale(self) -> float: ...
    @scale.setter
    def scale(self, value: float) -> None: ...
    @property
    def alpha(self) -> float: ...
    @alpha.setter
    def alpha(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Weibull:
    def __init__(self, shape: float = 1.0, scale: float = 1.0) -> None: ...
    @property
    def shape(self) -> float: ...
    @shape.setter
    def shape(self, value: float) -> None: ...
    @property
    def scale(self) -> float: ...
    @scale.setter
    def scale(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Rayleigh:
    def __init__(self, sigma: float = 1.0) -> None: ...
    @property
    def sigma(self) -> float: ...
    @sigma.setter
    def sigma(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class VonMises:
    def __init__(self, mu: float = 0.0, kappa: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def kappa(self) -> float: ...
    @kappa.setter
    def kappa(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Binomial:
    def __init__(self, n: int = 10, p: float = 0.5) -> None: ...
    @property
    def n(self) -> int: ...
    @n.setter
    def n(self, value: int) -> None: ...
    @property
    def p(self) -> float: ...
    @p.setter
    def p(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class NegativeBinomial:
    def __init__(self, r: float = 1.0, p: float = 0.5) -> None: ...
    @property
    def r(self) -> float: ...
    @r.setter
    def r(self, value: float) -> None: ...
    @property
    def p(self) -> float: ...
    @p.setter
    def p(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Geometric:
    def __init__(self, p: float = 0.5) -> None: ...
    @property
    def p(self) -> float: ...
    @p.setter
    def p(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Laplace:
    def __init__(self, mu: float = 0.0, b: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def b(self) -> float: ...
    @b.setter
    def b(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Cauchy:
    """Cauchy distribution. mean/variance/skewness/kurtosis return NaN (undefined)."""
    def __init__(self, x0: float = 0.0, gamma: float = 1.0) -> None: ...
    @property
    def x0(self) -> float: ...
    @x0.setter
    def x0(self, value: float) -> None: ...
    @property
    def gamma(self) -> float: ...
    @gamma.setter
    def gamma(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Logistic:
    def __init__(self, mu: float = 0.0, s: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def s(self) -> float: ...
    @s.setter
    def s(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Gumbel:
    def __init__(self, mu: float = 0.0, beta: float = 1.0) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def beta(self) -> float: ...
    @beta.setter
    def beta(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Erlang:
    def __init__(self, k: int = 1, lam: float = 1.0) -> None: ...
    @property
    def k(self) -> int: ...
    @k.setter
    def k(self, value: int) -> None: ...
    @property
    def lam(self) -> float: ...
    @lam.setter
    def lam(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class FisherF:
    def __init__(self, d1: float = 1.0, d2: float = 1.0) -> None: ...
    @property
    def d1(self) -> float: ...
    @d1.setter
    def d1(self, value: float) -> None: ...
    @property
    def d2(self) -> float: ...
    @d2.setter
    def d2(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class InverseGamma:
    """Inverse Gamma distribution. beta is a SCALE parameter (scipy invgamma(a, scale=beta))."""
    def __init__(self, alpha: float = 1.0, beta: float = 1.0) -> None: ...
    @property
    def alpha(self) -> float: ...
    @alpha.setter
    def alpha(self, value: float) -> None: ...
    @property
    def beta(self) -> float: ...
    @beta.setter
    def beta(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class HalfNormal:
    def __init__(self, sigma: float = 1.0) -> None: ...
    @property
    def sigma(self) -> float: ...
    @sigma.setter
    def sigma(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class Bernoulli:
    def __init__(self, p: float = 0.5) -> None: ...
    @property
    def p(self) -> float: ...
    @p.setter
    def p(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

class TruncatedNormal:
    def __init__(
        self,
        mu: float = 0.0,
        sigma: float = 1.0,
        a: float = ...,
        b: float = ...,
    ) -> None: ...
    @property
    def mu(self) -> float: ...
    @mu.setter
    def mu(self, value: float) -> None: ...
    @property
    def sigma(self) -> float: ...
    @sigma.setter
    def sigma(self, value: float) -> None: ...
    @property
    def a(self) -> float: ...
    @a.setter
    def a(self, value: float) -> None: ...
    @property
    def b(self) -> float: ...
    @b.setter
    def b(self, value: float) -> None: ...
    @overload
    def pdf(self, x: float) -> float: ...
    @overload
    def pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def log_pdf(self, x: float) -> float: ...
    @overload
    def log_pdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def cdf(self, x: float) -> float: ...
    @overload
    def cdf(self, x: NDArray[np.float64]) -> NDArray[np.float64]: ...
    @overload
    def ppf(self, p: float) -> float: ...
    @overload
    def ppf(self, p: NDArray[np.float64]) -> NDArray[np.float64]: ...
    def fit(self, data: NDArray[np.float64]) -> None: ...
    def sample(self, n: int = 1, seed: int | None = None) -> NDArray[np.float64]: ...
    def __repr__(self) -> str: ...
    @property
    def mean(self) -> float: ...
    @property
    def variance(self) -> float: ...
    @property
    def std(self) -> float: ...
    @property
    def skewness(self) -> float: ...
    @property
    def kurtosis(self) -> float: ...
    @property
    def name(self) -> str: ...
    @property
    def support(self) -> tuple[float, float]: ...
    @property
    def is_discrete(self) -> bool: ...
    @property
    def num_parameters(self) -> int: ...

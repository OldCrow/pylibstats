...
DISTRIBUTIONS = [
    ("Gaussian(0,1)", pylibstats.Gaussian(0.0, 1.0), sp.norm(0.0, 1.0), (-4.0, 4.0)),
    ("Exponential(1)", pylibstats.Exponential(1.0), sp.expon(scale=1.0), (0.01, 10.0)),
    ("Uniform(0,1)", pylibstats.Uniform(0.0, 1.0), sp.uniform(0.0, 1.0), (0.0, 1.0)),
    ("Gamma(2,1)", pylibstats.Gamma(2.0, 1.0), sp.gamma(a=2.0, scale=1.0), (0.01, 10.0)),
    ("Beta(2,5)", pylibstats.Beta(2.0, 5.0), sp.beta(a=2.0, b=5.0), (0.01, 0.99)),
    ("ChiSquared(5)", pylibstats.ChiSquared(5.0), sp.chi2(df=5), (0.1, 20.0)),
    ("StudentT(10)", pylibstats.StudentT(10.0), sp.t(df=10), (-5.0, 5.0)),
    ("LogNormal(0,1)", pylibstats.LogNormal(0.0, 1.0), sp.lognorm(s=1.0, scale=1.0), (0.01, 8.0)),
    ("Pareto(1,2)", pylibstats.Pareto(1.0, 2.0), sp.pareto(b=2.0, scale=1.0), (1.0, 20.0)),
    ("Weibull(2,1)", pylibstats.Weibull(2.0, 1.0), sp.weibull_min(c=2.0, scale=1.0), (0.01, 4.0)),
    ("Rayleigh(1)", pylibstats.Rayleigh(1.0), sp.rayleigh(scale=1.0), (0.01, 6.0)),
    (
        "VonMises(0,2)",
...
def run_benchmarks() -> None:
    header = (
        f"{'Distribution':>20s}  {'N':>10s}  {'Op':>4s}  "
    ...
    for name, pl_dist, sc_dist, (lo, hi) in DISTRIBUTIONS:
        for n in SIZES:
            ...
            print(
                f"{name:>20s}  {n:>10,d}  {'PDF':>4s}  {t_pl:>15.2f}  {t_sc:>12.2f}  {ratio:>7.1f}x"
            )
            ...
            ratio = t_sc / t_pl if t_pl > 0 else float("inf")
            print(
                f"{'':>20s}  {'':>10s}  {'CDF':>4s}  {t_pl:>15.2f}  {t_sc:>12.2f}  {ratio:>7.1f}x"
            )

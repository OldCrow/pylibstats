...
## In Progress [OPEN]
- **0.7.1 SHIPPED 2026-09-19** (travel session): PR #22 squash-merged
  (`70ea5d4`), main CI green, signed tag v0.7.1 pushed [user-approved],
  wheels green on all targets + sdist, PyPI 0.7.1 live, GitHub release
  published. Contents: mypy adopted (#6 closed); libstats pin v2.4.0 →
  v2.4.1, behavior-changing for Geometric/NegativeBinomial only (counts
  past INT_MAX in pmf, log_pdf, cdf, sample — libstats #125);
  `TestGeometricCountsBeyondIntMax` is two-sided against the closed forms
  (5/5 fail on a v2.4.0-source build, pass on the fix; suite 632 → 637).
